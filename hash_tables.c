...
// set to 1 to enable quadratic probing, or 0 for linear probing
#define QUADRATIC_PROBING 0
...
int get_pos(hash_table *h, char *key)
{
	if (QUADRATIC_PROBING)
		...
		return linear_get_pos(h, key);
}

// linear probing
int linear_get_pos(hash_table *h, char *key)
{
	int hval = hash(key, h->capacity);

	while (h->table[hval] != NULL)
	{
		...
		if (strcmp(h->table[hval], key) == 0)
			return hval;
		
		printf(" *%s* collides with *%s*\n", key, h->table[hval]);
		hval = (hval + 1) % h->capacity;
	}
